...
const CreateLaptopForm = () => {
  ...
  return (
    <Fragment>
              ...
              <Select multiple value={laptopData.gpus.map((gpu) => gpu.id)} onChange={handleGpuChange}
                ...
                {gpus.map((gpu) => (
                  <MenuItem key={gpu.id} value={gpu.id}
                    disabled={
                      !laptopData.gpus.some((selectedGpu) => selectedGpu.id === gpu.id) && isGpuTypeSelected(gpu.type, gpu.id)
                    }>
                    <Checkbox checked={laptopData.gpus.some((selectedGpu) => selectedGpu.id === gpu.id)} />
                    <ListItemText primary={`${gpu.model} (${gpu.type})`} />
                  </MenuItem>
          ...
          <Grid item xs={12} sm={6}>
            <FormControl fullWidth>
              ...
              <Select
                ...
                onChange={(e) => handleChange(e)} // Sử dụng handleChange để cập nhật state
                renderValue={(selected) =>
                  osVersions.find((os) => os.id === selected)?.version || "Select OS Version"
                }
                MenuProps={{ PaperProps: { style: { maxHeight: "50vh" } } }}
                label="OS version"
              >
              ...
              >
                {categories.map((category) => (
                  <MenuItem key={category.id} value={category.id}>
                    <Checkbox
                      checked={laptopData.categories.some((cat) => cat.id === category.id)}
                    />
      ...
      {error && (
        <Typography variant="h6" style={{ color: "red", marginTop: "20px" }}> {error} </Typography>
      )}
    </Fragment>
  );
};
